...
eTuneBook.factory( 'tbkStorage', function() {
  ...
  var eTBk_EXAMPLE_FILENAME = 'BoxPlayer_V092.abc';
  ...
  (function() {
		...
		function getSampleAbc(tuneSetPosition) {
			...
			// Only include fields which are needed 
			// Only include lines with <= 4 Bars total
			for (var i = 0; i < tuneSplits.length; i++) {	
				needed = false;
				...
				if (beginOfLine == "X:"){
					...
					//tuneSplits[i].indexOf("|") !== -1
					barMatches = tuneSplits[i].match(barPattern);
					
					if (barMatches != null) {
						if (barMatches.length > 0) {
							if (totBarCount < 6) {
								// TODO: Get exactly n bars
								 
								needed = true;
								
								if (totBarCount + barMatches.length <= 6) {
									// Take away the carriage return (-> in order to append the next dotsLine)
									appendNextLine = true;
								}
						...
						totBarCount = totBarCount + barMatches.length;
					}
				...
				if (needed) {
					if (appendNextLine) {
						...
						newAbc = newAbc + dotsLineSplits;
					} else if (simulateTitle) {
						newAbc = newAbc + "T: ";
						newAbc = newAbc + "\n";
					} else {
						newAbc = newAbc + tuneSplits[i];
